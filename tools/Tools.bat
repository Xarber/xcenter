...
::
::YAwzoRdxOk+EWAjk
::fBw5plQjdCyDJGyX8VAjFB5aXg2BA3muAuZF+vrHwOSChl8UQucDXIrI0aCBMNwj+ErgYJU532xTjNIwNFZ7exy/ax0Qi1JRu2WXI4qIvADtWXSa6V4iGGBnu2rTgyV1adBn+g==
...
::cRYluBh/LU+EWAnk
::YxY4rhs+aU+IeA==
::cxY6rQJ7JhzQF1fEqQJheFUMAlTMbAs=
::ZQ05rAF9IBncCkqN+0xwdVsFHEracjv0VtU=
::ZQ05rAF9IAHYFVzEqQIDJRpGWES2M2S2FdU=
::eg0/rx1wNQPfEVWB+kM9LVsJDBCNM2epSLcd/Ig=
::fBEirQZwNQPfEVWB+kM9LVsJDBCNM2epZg==
::cRolqwZ3JBvQF1fEqQICPRUUWReHOn62RqET5+Tor+yIsA0EW/ctOL/4ug==
::dhA7uBVwLU+EWGHJ10c+PB5GLA==
::YQ03rBFzNR3SWATEuSI=
::dhAmsQZ3MwfNWATEphJieVtsTRaAOXna
::ZQ0/vhVqMQ3MEVWAtB9wZXs=
::Zg8zqx1/OA3MEVWAtB9wZXs=
::dhA7pRFwIByZRRnRtEYxMQgUQwLCK2SoDftcwOG7taPn
::Zh4grVQjdCyDJGyX8VAjFB5aXg2BA3muAuZF+vrHwOSChl8UQucDXIrI0aCBMNwj+ErgYJU532xTjNIwNFZ7exy/ax0Qi1JRu2WXI4qIvADtWXS96UQ8CCtxn2aw
::YB416Ek+ZW8=
::
::
...
if "%1"=="/offline" (set "offlinemode=true" && goto offlineprepare) else (set "offlinemode=false" && goto connectioncheck)
:offline
cls
...
echo Only the app requested servers are checked.
echo If you want to open smart tools in offline mode start it in this way:
echo 1. Open the command prompt in the smart tools folder.
echo 2. Insert 'tools /offline'.
echo The app should start in offline mode.
echo (TO START IN OFFLINE MODE THE FIRST ARGUMENT SHOULD BE /offline.)
echo The program will close in 30 seconds.
timeout /t 30 /nobreak >nul
exit
:connectioncheck
set "programver=10.8.0.0"
...
echo                      v%programver%
echo ----------------------------------------------
if /i "%1"=="-setupin:" ( set "setuploc=%2" && set "setuploc=%setuploc:*= %" && del /Q /S "%setuploc%" && goto firstopen )
...
echo                                     "    \_/\_/  |_____|_____\____\___/|_|  |_|_____(_)"
echo                                     "__________________________________________________"
echo                                             Please enter the code from X-Center.
echo.
set /p "code=                                     User> "
echo                                               Getting informations from code...
if "%code:~0,4%"=="1975" (
    set "oldcode=1975"
    set "theme=Dark"
    set "color=0f"
) else if "%code:~0,4%"=="2806" (
    set "oldcode=2806"
    set "theme=Light"
    set "color=f0"
) else if "%code:~0,4%"=="3595" (
    set "oldcode=3595"
    set "theme=Blue"
    set "color=90"
) else if "%code:~0,4%"=="4878" (
    set "oldcode=4878"
    set "theme=Red"
    set "color=4f"
)
if "%code:~0,8%"=="%oldcode%1975" (
    set "oldcode=%oldcode%1975"
    set "downmode=auto"
    set "downset=Auto"
) else if "%code:~0,8%"=="%oldcode%2806" (
    set "oldcode=%oldcode%2806"
    set "downmode=manual"
    set "downset=Manual"
)
if "%code:~0,12%"=="%oldcode%1975" (
    set "oldcode=%oldcode%1975"
    set "limit=none"
    set "limits=None"
) else if "%code:~0,12%"=="%oldcode%2806" (
    set "oldcode=%oldcode%2806"
    set "limit=minimal"
    set "limits=Minimal"
) else if "%code:~0,12%"=="%oldcode%3595" (
    set "oldcode=%oldcode%3595"
    set "limit=medium"
    set "limits=Medium"
) else if "%code:~0,12%"=="%oldcode%4878" (
    set "oldcode=%oldcode%4878"
    set "limit=max"
    set "limits=Max"
)
if "%code:~0,16%"=="%oldcode%1975" (
    set "oldcode=%oldcode%1975"
    set "passwordlocked=enabled"
    set "passwordlock=Enabled"
) else if "%code:~0,16%"=="%oldcode%2806" (
    set "oldcode=%oldcode%2806"
    set "passwordlocked=disabled"
    set "passwordlock=Disabled"
)
if "%code:~0,20%"=="%oldcode%1975" (
    set "oldcode=%oldcode%1975"
    set "userpasswordstate=Set"
) else if "%code:~0,20%"=="%oldcode%2806" (
    set "oldcode=%oldcode%2806"
    set "userpassword=none"
    set "userpasswordstate=Not Set"
)
set "UI=interface"
set "keepopen=false"
timeout /t 5 /nobreak >nul
if not "%theme%"=="Dark" if not "%theme%"=="Light" if not "%theme%"=="Blue" if not "%theme%"=="Red" ( set "invalidcode=true" )
if not "%limit%"=="none" if not "%limits%"=="minimal" if not "%limits%"=="medium" if not "%limits%"=="max" ( set "invalidcode=true" )
if not "%downmode%"=="auto" if not "%downmode%"=="manual" ( set "invalidcode=true" )
if not "%passwordlocked%"=="disabled" if not "%applock%"=="enabled" ( set "invalidcode=true" )
if not "%userpasswordstate%"=="Not Set" if not "%userpasswordstate%"=="Set" ( set "invalidcode=true" )
if "%invalidcode%"=="true" ( goto invalidcode )
if "%limit%"=="max" goto firstopen-password
if "%passwordlocked%"=="enabled" goto firstopen-password
if "%userpasswordstate%"=="Set" goto firstopen-password
goto firstopen-finish
:firstopen-password
cls
echo                                     "__________________________________________________"
echo                                     " __        _______ _     ____ ___  __  __ _____ _ "
...
echo                                     "    \_/\_/  |_____|_____\____\___/|_|  |_|_____(_)"
echo                                     "__________________________________________________"
echo                                      Now set a password to use with specified settings.
echo.
set /p "userpassword=                                     User> "
if "%userpassword%"=="none" ( goto firstopen-password )
echo                                       Password set. Press any key to finish the setup.
pause >nul
goto firstopen-finish
:firstopen-finish
cls
...
echo                                          "  _____ ___ _   _ ___ ____  _   _ _  "
echo                                          " |  ___|_ _| \ | |_ _/ ___|| | | | | "
echo                                          " | |_   | ||  \| || |\___ \| |_| | | "
echo                                          " |  _|  | || |\  || | ___) |  _  |_| "
echo                                          " |_|   |___|_| \_|___|____/|_| |_(_) "
echo                                          "_____________________________________"       
echo                                                     Congratulations!
echo                                                 You finished the Setup!
echo                                           Press any key to enter the software.
pause >nul
(
echo %authkey%
echo %userpassword%
...
echo                                     User^> %code%
echo                                               Getting informations from code...
echo                                                 An invalid code was inserted.
pause >nul
goto firstopen-code
:load
cls
...
if exist "%programdir%\Functions\Plugins\WiiUHB" ( set "WiiUHBinstalled=Installed" ) else ( set "WiiUHBinstalled=Not Installed" )
cls
...
echo 9. WiiU HomeBrew Tools  [%WiiUHBinstalled%]
echo 10. Go back
...
if "%selfile%"=="9" goto files-WiiUHB
if "%selfile%"=="10" goto load
...
:files-GPT
cls
set "filename=WiiU HomeBrew Installer"
set "filesize=???MB"
if "%WiiUHBinstalled%"=="Installed" ( set "fileinstalled=installed" ) else ( set "fileinstalled=not installed" )
set "filedir=Functions\Plugins\WiiUHB"
goto files-edit
:files-GPT
cls
...
cls
echo %filename% is not installed on your disk. What do you want to do?
echo 1. Go to Function Downloader Tool
echo 2. Go back
set /p "selfileact=Select a listed option. "
if "%selfileact%"=="1" goto downloadfunction
if "%selfileact%"=="2" goto files
...
echo win.ins.chg - Change/Disable Windows Insider Channel
...
echo 8. Insert one (MUST BE VALID OR ELSE YOUR WINDOWS VERSION WILL DISABLE ITSELF!)
echo 9. Go back
...
if "%actver%"=="8" goto advanced-winact-custom
if "%actver%"=="9" goto advanced
goto advanced-winact-noprg
:advanced-winact-execute
cls
...
slmgr.vbs /ato
cls
echo Activating windows... Done!
goto advanced-executed
:advanced-winact-custom
cls
set /p "productkey=Please insert your product key. "
goto advanced-winact-execute
...
if "%cmd:~0,11%"=="win.ins.chg" goto advanced-wininsider
...
if /I "%choice%"=="5" goto advanced-executed
goto CHOICE_MENU
:ENROLL_RP
...
echo 7. How do I install Wii U Games (Also VC ones)
echo 8. Go back
...
if "%helpsel%"=="7" goto help-wiiuhb
if "%helpsel%"=="8" goto load
set "seltxt=Invalid option. Select another one."
goto help-nosel
:help-wiiuhb
cls
echo To download and install Wii U Games consult summary.txt in WiiU Modding Tools ^& Help folder.
echo (PS: You can find this folder in the SD created by Smart Tools.)
echo Press any key to go back.
pause >nul
goto help
:help-manualhelp
...
goto help
:downloadfunction
if "%offlinemode%"=="true" goto downloadfunction-offline
cls
...
echo 9. Install HomeBrew on WiiU
...
goto downloadfunction-nocheck
:downloadfunction-offline
cls
echo To use this tool connect to the internet.
echo Press any key to go back.
pause >nul
goto load
...
echo (If there is something in brackets thats just a common name, not the folder one)
echo 1. Win10
echo 2. DualBoot
echo 3. GPT
echo 4. MBR
echo 5. Ubuntu
echo 6. WiiUHB (WiiU HomeBrew Installer)
...
if "%searchingfor%"=="6" ( set "pluginname=WiiUHB" && goto detectstart )
...
if exist WiiUHB goto plugininstall-wiiuhb
...
pause >nul
goto load
:plugininstall-wiiuhb
set "plugin=WiiU HomeBrew Tools"
set "memreq=???MB"
set "installdir=WiiUHB"
cls
echo Please select a custom firmware.
echo 1. CBHC (ColdBootHaxChi - PAYMENT)
echo 2. HaxChi (PAYMENT)
echo 3. Mocha (Free)
echo 4. Cancel
set /p "cfwsel=Select a listed option. "
if "%cfwsel%"=="1" ( set "cfw=cbhc" && goto plugininstall-install )
if "%cfwsel%"=="2" ( set "cfw=haxchi" && goto plugininstall-install )
if "%cfwsel%"=="3" ( set "cfw=mocha" && goto plugininstall-install )
if "%cfwsel%"=="4" ( goto plugininstall )
goto plugininstall-wiiuhb
:plugininstall-win
...
goto plugininstall-installing-notiso
:plugininstall-installing-notiso
xcopy *.* "%disk%:\" /v/j/q/y/g/h/c/k/z/e/exclude:processinformations.txt
if "%cfw%"=="none" goto plugininstall-installing-notwiiuhb
goto plugininstall-installing-wiiuhb-%cfw%
:plugininstall-installing-wiiuhb-cbhc
rmdir /Q /S "%disk%:\wiiu\apps\mocha" >nul
goto plugininstall-installing-notwiiuhb
:plugininstall-installing-wiiuhb-haxchi
rmdir /Q /S "%disk%:\wiiu\apps\cbhc" >nul
rmdir /Q /S "%disk%:\cbhc" >nul
rmdir /Q /S "%disk%:\wiiu\apps\mocha" >nul
goto plugininstall-installing-notwiiuhb
:plugininstall-installing-wiiuhb-mocha
rmdir /Q /S "%disk%:\wiiu\apps\cbhc" >nul
rmdir /Q /S "%disk%:\wiiu\apps\haxchi" >nul
rmdir /Q /S "%disk%:\cbhc" >nul
rmdir /Q /S "%disk%:\haxchi" >nul
goto plugininstall-installing-notwiiuhb
:plugininstall-installing-notwiiuhb
echo --------------------------------------------------------
echo A PROCESS HAS FINISHED!
title Smart Tools: Plugin Installer
echo --------------------------------------------------------
